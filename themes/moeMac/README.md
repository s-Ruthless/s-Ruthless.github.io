...
| 💬 多评论系统 | Giscus / Waline / Twikoo / Gitalk / CWD，暗黑模式自动切换 |
| 🔗 社交链接 | 首页展示 GitHub / 邮箱 / QQ / 微信等社交按钮（内联 SVG 图标） |
...
| 评论系统 | Giscus / Waline / Twikoo / CWD 均为前端加载 |
| 暗黑模式 | CSS + localStorage |
| AJAX 导航 | 前端 JS |
| 社交链接 | 内联 SVG 图标，不依赖外部图标字体 |
...
| `qq` | QQ | `tencent://message/?uin=QQ号&Site=&Menu=yes` |
| `wechat` | 微信 | 链接地址或 `#` |
| `twitter` | Twitter / X | `https://twitter.com/用户名` |
| `bilibili` | 哔哩哔哩 | `https://space.bilibili.com/UID` |
...
支持 Giscus / Waline / Twikoo / Gitalk / CWD，选择一种配置即可。
...
[CWD](https://cwd.js.org/) 是一个基于 Cloudflare Workers 的轻量评论系统，部署简单、免费。

1. 前往 [CWD 文档](https://cwd.js.org/) 部署 API 到 Cloudflare Workers
2. 填写：
...
comments:
  ...
  provider: "cwd"
  cwd:
    apiBaseUrl: "https://your-cwd-api.workers.dev"  # CWD API 地址
    siteId: "blog"    # 站点标识，用于多站点数据隔离
    lang: "auto"      # 评论组件语言，auto 自动检测浏览器语言
```

> CWD 使用 Shadow DOM 渲染，主题会自动注入样式以适配站点配色和暗黑模式。
...
douban:
  ...
  sync_interval_hours: 24     # 自动同步间隔（小时），0 = 每次 hexo g 都同步
...
开启后，每次 `hexo generate` / `hexo server` 会自动检查上次同步时间，距上次同步超过 `sync_interval_hours` 才触发，不会每次 build 都等待。设为 `0` 则每次 `hexo g` 都同步。
...
- **Giscus / Waline / Twikoo / Gitalk / CWD** — 评论系统
...
社交按钮使用内联 SVG 图标，支持 `github` / `email` / `qq` / `wechat` / `twitter` / `bilibili` / `zhihu` / `link` 类型。请确认 `type` 值拼写正确，不在列表中的类型会显示通用链接图标。
...
- [CWD](https://cwd.js.org/) — 基于 Cloudflare Workers 的评论系统
