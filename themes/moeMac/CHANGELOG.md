...
- **新增 4 种布局** — 在原有 grid/list/simple/magazine/card/compact 基础上，新增 timeline（时间线）、overlay（封面叠加）、feature（特色卡片）、zigzag（Z字形）共 10 种布局
- **布局配置** — `posts_layout` 配置项支持 10 种布局值，在主题 `_config.yml` 中一键切换
- **时间线布局** — 按年份分组，竖向时间轴排列，适合日记型/归档展示
- **封面叠加布局** — 大图背景+底部渐变遮罩+文字叠加，Medium/Substack 风格，首篇横跨全宽
- **特色卡片布局** — 首个横向大卡片（图左文右）+ 后续 3 列网格，Apple News 风格
- **Z字形布局** — 左右交替排列的横向卡片，设计博客/产品展示风格
- **所有布局共享** — 封面图懒加载（`loading="lazy"` + 小猫加载动画）、置顶徽章、分类筛选、响应式适配、AJAX 无刷新跳转

### 移除 Masonry 依赖

- **移除 Masonry JS 库** — 删除 `masonry.pkgd.min.js`，文章列表改用纯 CSS Grid/Flexbox/Columns 实现
- **移除 Masonry 引用** — `scripts.ejs` 不再加载 Masonry，`main.js` 移除相关初始化代码
- **保留相册瀑布流** — 相册页面的 `gallery-masonry`（纯 CSS columns）不受影响

### 布局优化

- **移除"阅读全文"** — 所有布局中的"阅读全文"链接已移除，卡片整体可点击跳转
- **杂志布局改进** — 所有 sticky 文章均作为 hero 大卡片展示（原来仅第一篇）
- **封面加载动画统一** — 新布局共享小猫奔跑加载动画
- **暗黑模式适配** — 新布局均适配暗黑模式

### 文档更新

- **README.md** — 特性列表新增多布局、移除 Masonry 引用、修正文档链接 abbrlink、致谢移除 Masonry
- **使用说明** — 新增「文章列表布局」章节，包含 10 种布局表格、特性说明、选择建议、配置示例
